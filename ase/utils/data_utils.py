...
from .general_utils import get_score_vector_positions
from .general_utils import get_scaled_down_scores, pad_hierarchical_text_sequences, get_attribute_masks
url_replacer = '<url>'
...
def get_feature(sample, pos_tags, read_configs):
    readability_features = get_readability_features(read_configs.READABILITY_PATH)
    linguistic_features = get_linguistic_features(read_configs.FEATURES_PATH)
    normalized_linguistic_features = get_normalized_features(linguistic_features)
    data = read_essay_sets([sample], readability_features, normalized_linguistic_features, pos_tags)
    data['y_scaled'] = get_scaled_down_scores(data['data_y'], data['prompt_ids'])
    X_pos = pad_hierarchical_text_sequences(data['pos_x'], read_configs.MAX_SENTNUM, read_configs.MAX_SENTLEN)
    X_pos = X_pos.reshape((X_pos.shape[0], X_pos.shape[1] * X_pos.shape[2]))#.astype(np.float32)
    X_linguistic_features = np.array(data['features_x'])#.astype(np.float32)
    X_readability = np.array(data['readability_x'])#.astype(np.float32)
    Y = np.array(data['y_scaled'])
    return X_pos, X_linguistic_features, X_readability, Y
